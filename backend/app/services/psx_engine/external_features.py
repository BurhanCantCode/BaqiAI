...
# TradingView scraper (not available in BAQI AI context)
...
# Cache settings
# Navigate up from psx_engine/ -> services/ -> app/ -> backend/ -> data/
CACHE_DIR = Path(__file__).parent.parent.parent.parent / "data" / "external_cache"
...
def merge_external_features(stock_df: pd.DataFrame, 
    ...
    # Normalize to ns precision to avoid merge_asof dtype mismatches with yfinance
    ...
    if not usdpkr.empty:
        usdpkr['date'] = pd.to_datetime(usdpkr['date']).dt.tz_localize(None).astype('datetime64[ns]')
        df = pd.merge_asof(
            df.sort_values('Date'),
    ...
    if symbol and symbol.upper() != 'KSE100':
        ...
        if not kse100.empty:
            kse100['date'] = pd.to_datetime(kse100['date']).dt.tz_localize(None).astype('datetime64[ns]')
            df = pd.merge_asof(
                df.sort_values('Date'),
    ...
    if not commodities.empty:
        commodities['date'] = pd.to_datetime(commodities['date']).dt.tz_localize(None).astype('datetime64[ns]')
        df = pd.merge_asof(
            df.sort_values('Date'),
